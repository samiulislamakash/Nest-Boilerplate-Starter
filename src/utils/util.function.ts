...
function generateFilename(file) {
	const fileType: string = file.originalname.split('.')[file.originalname.split('.').length - 1];
	return `${Date.now()}.${fileType}`;
}

export function filterEntityFieldsFromDto(repository: any, dto: any) {
	let filteredDto = {};

	const fields = repository.metadata.ownColumns.map((d) => d.propertyName);
	const keys = Object.keys(dto);

	for (const key of keys) {
		if (fields.indexOf(key) != -1) {
			if (dto[key] == null || dto[key] == undefined || dto[key]?.length == 0) continue;
...
export function getMonthRangeWithIndexFromStartDateAndEndDate(startDateString: string, endDateString: string) {
	let response = {};
	let index = 0;
...
export function getYearsArrayFromStartDateAndEndDate(startDate: any, endDate: any) {
	let years = [];
...
export function getValidNumberOrZero(number: any) {
	if (number != null && number != undefined && number != Infinity && !isNaN(number)) return number;
	else return 0;
}
...
export function isSameDate(date1, date2): boolean {
	return (
		new Date(new Date(date1).setHours(0, 0, 0, 0)).getTime() == new Date(new Date(date2).setHours(0, 0, 0, 0)).getTime()
	);
}
