...
import { Entity, In, Repository, DataSource, FindOneOptions } from 'typeorm';
import {
  filterDtoFieldsFromObject,
  getPaginationParameters,
} from 'src/utils/util.function';
...
export abstract class CommonService<Entity> extends Repository<Entity> {
  ...
  async _bulkCreate(payload: any, relations?: string[]) {
    try {
      return await this._repository.manager.transaction(
        async (transactionalEntityManager) => {
          const save_res = await transactionalEntityManager.save(
          ...
          return await transactionalEntityManager.find(
            this._repository.metadata.target,
            {
              where: { id: In([...ids]) },
              relations: relations,
  ...
  async _filter(param: any, relations?: string[]) {
    try {
      ...
      const filterParams = filterDtoFieldsFromObject(
        this._repository.metadata.propertiesMap,
        param,
      );
